...
def test_thundering_herd():
    ...
    s = connect()
    if not s:
        print("Skipping thundering herd (connection failed)")
        return
    ...
    def client_task(i):
        ...
        c = None
        try:
            c = connect(retries=1)  # Don't retry too hard inside herd
            if not c:
                return
            f = c.makefile("rb")
    ...
    for t in threads:
        t.join(timeout=5.0)  # Ensure we don't hang forever

    print(
        f"Thundering Herd finished. Successful transactions: {success_count}/{CLIENTS}"
    )
