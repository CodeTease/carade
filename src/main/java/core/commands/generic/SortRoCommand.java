...
public class SortRoCommand implements Command {
    @Override
    public void execute(ClientHandler client, List<byte[]> args) {
        if (args.size() < 2) {
            client.sendError("usage: SORT_RO key [BY pattern] [LIMIT offset count] [GET pattern [GET pattern ...]] [ASC|DESC] [ALPHA]");
            return;
        }
        ...
        for (int i = 2; i < args.size(); i++) {
            ...
            if (arg.equals("BY") && i + 1 < args.size()) {
                ...
                client.sendError("ERR syntax error, STORE option is not allowed in SORT_RO");
                return;
            }
        }
    ...
    private String getValue(int dbIndex, String key) {
        ...
        if (hashField != null) {
            ...
            if (entry.getValue() instanceof core.structs.CaradeHash) {
                map = ((core.structs.CaradeHash) entry.getValue()).map;
            } else {
                map = (ConcurrentHashMap<String, String>) entry.getValue();
            }
