...
pub async fn run(client: redis::Client, id: usize, requests: usize) -> anyhow::Result<BenchStats> {
    ...
    let payload_10kb: String = (0..size_10kb).map(|_| rng.sample(Alphanumeric) as char).collect();
    let payload_100kb: String = (0..size_100kb).map(|_| rng.sample(Alphanumeric) as char).collect();
